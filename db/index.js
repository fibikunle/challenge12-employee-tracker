...
function getEmployeesToDelete() {
  mysqlConnection.query(
    ...
    function (err, results) {
      ...
      inquirer.prompt(deleteEmployeeQs).then((response) => {
...
function getRolesToDelete() {
  mysqlConnection.query(
    ...
    function (err, results) {
      ...
      inquirer.prompt(deleteRoleQs).then((response) => {
  ...
  mysqlConnection.query(
    ...
    function (err, results) {
      ...
      for (const department in results) {
        if (
          departmentsArr.indexOf(results[department].department_name) === -1
        ) {
          departmentsArr.push(results[department].department_name);
        }
      }
      inquirer.prompt(deleteDepartmentQs).then((response) => {
...
function updateManager(name, manager) {
  ...
  mysqlConnection.query(
    "UPDATE employee SET manager_id = (SELECT a.id FROM (SELECT b.id FROM employee b WHERE b.first_name = ? AND b.last_name = ?) a) WHERE first_name = ? AND last_name = ?;",
    [managerName[0], managerName[1], employeeName[0], employeeName[1]],
    function (err, results) {
      if (err) {
        console.log(err);
      } else {
        console.table(results);
        console.log(
          "Employee's manager has been successfully updated to database."
        );
        start.start();
      }
    }
...
function updateManagerPrompt() {
  inquirer.prompt(updateEmployeeManagerQs).then((response) => {
...
function selectManager() {
  ...
  mysqlConnection.query(
    'SELECT concat(b.first_name, " ", b.last_name) AS manager_name FROM employee a LEFT JOIN employee b ON a.manager_id = b.id INNER JOIN roles c ON a.role_id = c.id INNER JOIN department d ON d.id = c.department_id WHERE b.first_name IS NOT NULL AND b.last_name IS NOT NULL;',
    function (err, results) {
...
function updateEmployee(name, role) {
  const employeeName = name.split(" ");
  mysqlConnection.query(
    "UPDATE employee SET role_id = (SELECT r.id FROM roles r WHERE r.role_title = ?) WHERE employee.first_name = ? AND employee.last_name = ?;",
    [role, employeeName[0], employeeName[1]],
    function (err, results) {
      if (err) {
        console.log(err);
      } else {
...
function addEmployee(fName, lName, role, manager) {
  ...
  mysqlConnection.query(
    "INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES (?, ?, (SELECT r.id FROM roles r WHERE r.role_title = ?), (SELECT e.id FROM employee e WHERE e.first_name = ? AND e.last_name = ?));",
    [fName, lName, role, managerName[0], managerName[1]],
    function (err, results) {
      console.log(err);
      console.log(results);
...
function addRole(role, salary, department) {
  mysqlConnection.query(
    "INSERT INTO roles (role_title, salary, department_id) VALUES (?, ?, (SELECT id FROM department WHERE department_name = ?));",
    [role, Number(salary), department],
    function (err, results) {
      console.log(results);
...
function addDepartmentPrompt() {
  inquirer.prompt(addDepartmentQ).then((response) => {
...
function departmentPromptToView() {
  inquirer.prompt(getDepartmentQ).then((response) => {
...
function managerPrompt() {
  inquirer.prompt(getManagerQ).then((response) => {
...
function viewAllEmployees() {
  mysqlConnection.query(
    "SELECT a.id AS employee_id, a.first_name, a.last_name, c.role_title, d.department_name, c.salary, b.first_name AS manager_firstname, b.last_name AS manager_lastname FROM employee a LEFT JOIN employee b ON a.manager_id = b.id INNER JOIN roles c ON a.role_id = c.id INNER JOIN department d ON d.id = c.department_id;",
    function (err, results) {
      console.table(results);
      start.start();
    }
...
function viewAllRoles() {
  mysqlConnection.query(
    "SELECT roles.id AS role_id, roles.role_title, roles.salary, department.department_name FROM roles JOIN department ON roles.department_id = department.id;",
    function (err, results) {
      console.table(results);
      start.start();
    }
